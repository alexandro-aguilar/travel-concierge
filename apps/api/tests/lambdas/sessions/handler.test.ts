...
import { createHandler } from '../../../src/lambdas/sessions/handler.js';
...
const structured = (result: unknown): { readonly statusCode: number; readonly body: string } =>
  result as { readonly statusCode: number; readonly body: string };
describe('sessions HTTP handler', () => {
  ...
  it('creates a session and returns its public contract', async () => {
    const result = structured(
      await createHandler(dependencies())(event('POST', '/sessions'), {} as never, () => undefined),
    );
    expect(result.statusCode).toBe(201);
    expect(JSON.parse(result.body)).toMatchObject({ status: 'COLLECTING_REQUIREMENTS' });
  });
  it('trims accepted messages and rejects malformed or oversized input', async () => {
    const deps = dependencies();
    const handler = createHandler(deps);
    await handler(event('POST', '/sessions'), {} as never, () => undefined);
    const ok = structured(
      await handler(
        event(
          'POST',
          '/sessions/00000000-0000-4000-8000-000000000001/messages',
          '{"message":"  hello  "}',
        ),
        {} as never,
        () => undefined,
    ...
    expect(JSON.parse(ok.body).message.content).toBe('hello');
    const bad = structured(
      await handler(
        event('POST', '/sessions/00000000-0000-4000-8000-000000000001/messages', '{'),
        {} as never,
        () => undefined,
      ),
    );
  ...
  it('returns safe 404 and 500 error contracts', async () => {
    const result = structured(
      await createHandler(dependencies())(
        event('GET', '/sessions/00000000-0000-4000-8000-000000000001/trip'),
        {} as never,
        () => undefined,
      ),
    );
