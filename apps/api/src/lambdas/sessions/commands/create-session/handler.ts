...
export class CreateSessionHandler {
  ...
  public async execute(requestId: string): Promise<Pick<SessionMetadata, 'sessionId' | 'status' | 'createdAt'>> {
    return this.telemetry.span('createSession', { requestId }, async () => {
      const sessionId = this.ids.uuid();
      const createdAt = this.clock.now().toISOString();
      const metadata: SessionMetadata = { sessionId, status: 'COLLECTING_REQUIREMENTS', createdAt, updatedAt: createdAt, version: 1 };
