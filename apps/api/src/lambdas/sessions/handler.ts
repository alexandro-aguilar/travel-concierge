...
import { z } from 'zod';
import type { AppendMessageHandler } from './commands/append-message/handler.js';
import type { CreateSessionHandler } from './commands/create-session/handler.js';
...
const defaultDependencies: Dependencies = {
  create: defaultContainer.get(sessionsTypes.create),
...
const errorResponse = (requestId: string, error: unknown) => {
  if (error instanceof z.ZodError || error instanceof InvalidCursorError)
    return response(400, { code: 'INVALID_REQUEST', message: 'The request is invalid', requestId });
  if (error instanceof SessionNotFoundError)
    return response(404, { code: 'SESSION_NOT_FOUND', message: 'Session not found', requestId });
  if (error instanceof InvalidTripStateError || error instanceof ConditionalWriteConflictError)
    return response(409, { code: 'CONFLICT', message: 'The session cannot be updated', requestId });
  return response(500, {
    code: 'INTERNAL_ERROR',
...
export const createHandler =
  (dependencies: Dependencies): APIGatewayProxyHandlerV2 =>
  async (event) => {
      ...
      if (event.requestContext.http.method === 'GET' && !match[2]) {
        const cursor =
          event.queryStringParameters?.cursor === undefined
            ? undefined
            : cursorSchema.parse(event.queryStringParameters.cursor);
