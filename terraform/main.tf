locals { name = "travel-concierge-${var.environment}" }
...
resource "aws_cloudwatch_log_group" "sessions" { name = "/aws/lambda/${local.name}-sessions" retention_in_days = 30 }
...
resource "aws_lambda_function" "sessions" {
  ...
  environment { variables = { SESSIONS_TABLE_NAME = aws_dynamodb_table.sessions.name, SESSION_TTL_DAYS = tostring(var.session_ttl_days), PROVIDER_MODE = "mock" } }
  depends_on = [aws_cloudwatch_log_group.sessions]
}
resource "aws_apigatewayv2_api" "http" { name = "${local.name}-http" protocol_type = "HTTP" }
resource "aws_apigatewayv2_integration" "sessions" { api_id = aws_apigatewayv2_api.http.id integration_type = "AWS_PROXY" integration_uri = aws_lambda_function.sessions.invoke_arn payload_format_version = "2.0" }
...
resource "aws_lambda_permission" "api" { statement_id = "AllowHttpApiInvoke" action = "lambda:InvokeFunction" function_name = aws_lambda_function.sessions.function_name principal = "apigateway.amazonaws.com" source_arn = "${aws_apigatewayv2_api.http.execution_arn}/*/*" }
